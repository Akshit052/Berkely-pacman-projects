...
def constructBayesNet(gameState):
    ...
    "*** YOUR CODE HERE ***"
    # Constructing the empth bayes net as structure defined
    ...
    #Creating edges as shown in figure
...
def fillYCPT(bayesNet, gameState):
    ...
    "*** YOUR CODE HERE ***"
    #Filling YCPT
    #So in YCPT we setting it for the factors which are affecting Y
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    "*** YOUR CODE HERE ***"
    #Computing the marginal distribution over position using inferencebyVariableElimination
    factor = inference.inferenceByVariableElimination(bayesNet, [FOOD_HOUSE_VAR], evidence, eliminationOrder)
    return max(factor.getAllPossibleAssignmentDicts(), key = factor.getProbability)
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        "*** YOUR CODE HERE ***"
        #In part a
        # Computing the expected value of topright and topleft
        factor = inference.inferenceByVariableElimination(self.bayesNet, [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR]
        ...
        #Finding leftExpected Value
        leftExpectedValue = factor.getProbability(topleft) * WON_GAME_REWARD \
                            + factor.getProbability(topright) * GHOST_COLLISION_REWARD

        #Finding rightExpected Value
        rightExpectedValue = factor.getProbability(topright) * WON_GAME_REWARD \
                            + factor.getProbability(topleft) * GHOST_COLLISION_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        # In 8 b
        # Computing expected value of all hidden cells
        factor = inference.inferenceByVariableElimination(self.bayesNet, [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR]
            , evidence, enterEliminationOrder)

        # Taking decision according to expected value
        best_val = 0
        for prob, new_evidence in self.getExplorationProbsAndOutcomes(evidence):
            expectedValue += prob * max(self.computeEnterValues(new_evidence, enterEliminationOrder))
