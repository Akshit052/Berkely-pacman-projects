# Group 15-Akshit(1903106) Garg ,Aryan Yadav(1903107) ,Vivek Kaushik(1903140)
# multiAgents.py
# --------------
...
class MinimaxAgent(MultiAgentSearchAgent):
	...
	def min_value(self, gameState, depth, agent_idx):
		"""
			We have again done the same thing as max function to calculate the min value.	
		"""
	...
	def max_value(self, gameState, depth, agent_idx):
		"""
			Function to go through the whole game-state tree to determine the max value. 
		"""
		value = (float('-Inf'), None)
		legal_actions = gameState.getLegalActions(agent_idx)
		for action in legal_actions:
	...
	def value(self, gameState, depth, agent_idx):
		"""
			Function to determines whom to choose. 	
		"""
...
class AlphaBetaAgent(MultiAgentSearchAgent):
	...
	def is_terminal_state(self, gameState, depth, agent_idx):
		"""
			Function to see if we have reached a leaf node 
		"""
	...
	def min_value(self, gameState, depth, agent_idx, alpha, beta):
		"""
		Adding the beta factor for pruning in min value function.
		"""
		value = (float('+Inf'), None)
		legal_actions = gameState.getLegalActions(agent_idx)
		for action in legal_actions:
	...
	def max_value(self, gameState, depth, agent_idx, alpha, beta):
		"""
			Adding the alpha factor for pruning in max value function.
		"""
		value = (float('-Inf'), None)
		legal_actions = gameState.getLegalActions(agent_idx)
		for action in legal_actions:
	...
	def value(self, gameState, depth, agent_idx, alpha, beta):
		"""
		Again to determine whose chance it is
		"""
...
class ExpectimaxAgent(MultiAgentSearchAgent):
	...
	def is_terminal_state(self, gameState, depth, agent_idx):
		"""
			Function to determine if we reached a leaf node in the state search tree
		"""
	...
	def max_value(self, gameState, depth, agent_idx):
		...
		"""
			Again to calculate the max values
		"""
	...
	def expected_value(self, gameState, depth, agent_idx):
		"""
			Function to calculate the average or expected value.
		"""
		value = list()
		legal_actions = gameState.getLegalActions(agent_idx)
		for action in legal_actions:
...
def betterEvaluationFunction(currentGameState):
	"""
	  ...
	  DESCRIPTION: For this we have to take into account the following factors:
